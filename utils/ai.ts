...
export async function draftProtocol(projectData: any): Promise<string> {
  const prompt = `
    You are a Quality Improvement (QI) Academic Consultant.
    Draft a concise QI project protocol based on the following project details.
    Include: Background, Aim, Methods, Outcome Measures, and PDSA Plan.
    ...
    Keep the protocol professional and suitable for IRB submission. Use SQUIRE 2.0 guidelines.
  `;
  return askAI(prompt);
}
